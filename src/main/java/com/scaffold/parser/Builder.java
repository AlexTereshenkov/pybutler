...
src/main/java/nl/bigo/pythonparser/Builder.java
*/
            ...
            while (!childListStack.isEmpty()) {
                ...
                if (childStack.isEmpty()) {
                    ...
                    if (tree instanceof TerminalNode && tree.getText().trim().isEmpty()) {
                        text = "<"
                                + Python3Parser.tokenNames[((TerminalNode) tree).getSymbol().getType()]
                                + ">";
                    } else {
                        text = tree.getText().replace("\r", "").replace("\n", "\\n");
                    }

                    builder.append(indent).append(childStack.isEmpty() ? "'- " : "|- ")
                            .append(node.startsWith("terminal") ? text : node).append("\n");
